...
function renderFlights() {
  ...
  $("#flight-dots").innerHTML = journeys.map((_, index) => `<button type="button" class="carousel-dot" aria-label="查看第 ${index + 1} 组航班" aria-controls="flight-carousel" data-flight-index="${index}"></button>`).join("");
  ...
  function updateControls() {
    ...
    $("#flight-prev").disabled = activeIndex === 0;
    $("#flight-next").disabled = activeIndex === cards.length - 1;
  }
  function selectFlight(index) {
    ...
    carousel.scrollTo({ left: carousel.scrollLeft + cardBounds.left - bounds.left - (carousel.clientWidth - cardBounds.width) / 2, behavior: "smooth" });
  }
  $("#flight-prev").onclick = () => selectFlight(activeIndex - 1);
  $("#flight-next").onclick = () => selectFlight(activeIndex + 1);
  $("#flight-dots").onclick = (event) => {
    const dot = event.target.closest("button[data-flight-index]");
    if (dot) selectFlight(Number(dot.dataset.flightIndex));
  };
  updateControls();
  let scheduled = false;
  carousel.addEventListener("scroll", () => {
    ...
    requestAnimationFrame(() => {
      const bounds = carousel.getBoundingClientRect();
      const center = bounds.left + carousel.clientWidth / 2;
      let distance = Infinity;
      cards.forEach((card, index) => {
        const rect = card.getBoundingClientRect();
        const delta = Math.abs(rect.left + rect.width / 2 - center);
        if (delta < distance) { distance = delta; activeIndex = index; }
      });
      updateControls();
      scheduled = false;
    });
...
function renderRental() {
  ...
  if (rental.placeholder) {
    $("#rental-provider-label").textContent = "资料待补充";
    $("#rental-card").innerHTML = `<article class="rental-panel"><div class="rental-details"><h3>租车信息待补充</h3><p>本示例以铁路为主，尚未安排租车。租车公司、车型、取还车时间与费用待补充。</p></div></article>`;
    $("#drive-notes").innerHTML = "";
    return;
  }
  ...
  $("#rental-card").innerHTML = `
    <article class="rental-panel">
      <div class="return-deadline">
        ...
        <small>建议 ${escapeHtml(rental.dropoff.recommendedArrivalTime)} 抵达还车区域，预留验车及交接时间。</small>
      </div>
...
function updateRentalCountdown() {
  if (state.data.groundTransport.rentalCar.placeholder) return;
...
async function init() {
  try {
    ...
    if (moduleEnabled("ledger")) {
      if (state.data.initialLedger && state.config.persistence.mode === "local") {
        const adapter = window.TravelRuntimeStorage.createAdapter({ mode: "local", tripId: state.data.metadata.tripId, collections: ["settings", "travelers", "bills"] });
        const saved = await adapter.load();
        if (!saved?.settings && !saved?.travelers?.length && !saved?.bills?.length) {
          await adapter.save(state.data.initialLedger);
        }
      }
